      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-blue-600 via-blue-700 to-violet-700 flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="z-10">
          <div className="flex flex-col gap-4 mb-10">
            {[
              { grade: 'A+', label: 'Senior Engineer at Stripe', score: 94 },
              { grade: 'B+', label: 'Full-Stack Dev at Shopify', score: 78 },
            ].map(item => (
              <div key={item.grade} className="bg-white/10 backdrop-blur rounded-2xl p-4 flex items-center gap-4">
                <div className="text-2xl font-black text-emerald-300 w-10">{item.grade}</div>
                <div className="flex-1">
                  <div className="text-white text-sm font-medium mb-1.5">{item.label}</div>
                  <div className="bg-white/20 rounded-full h-1.5">
                    <div className="bg-emerald-400 h-1.5 rounded-full" style={{ width: `${item.score}%` }} />
                  </div>
                </div>
                <div className="text-emerald-300 text-sm font-bold">{item.score}%</div>
              </div>
            ))}
          </div>
          ...
          <p className="text-blue-200 text-base">
            AI-powered CV matching. Land more interviews.
          </p>
        </div>
      </div>
